...
{
	CurrentThread->OwnerProcess->GetThread(CreateThread(&TestTwoUI, THREADTYPE_UI))->Start();
...
{
	CurrentThread->OwnerProcess->GetThread(CreateThread(&TestThreeOne, THREADTYPE_NORM))->Start();
	CurrentThread->OwnerProcess->GetThread(CreateThread(&TestThreeTwo, THREADTYPE_NORM))->Start();
	CurrentThread->OwnerProcess->GetThread(CreateThread(&TestThreeThree, THREADTYPE_NORM))->Start();
...
__attribute__((noinline)) void TestFour()
{
	for(int j = 0; j < 10; j++)
	{
		for(int i = 0; i < 10; i++)
		{
			...
			int Test = CreateThread(&TestFourProgram, THREADTYPE_NORM);

...
{
	CurrentThread->OwnerProcess->GetThread(CreateThread(&TestThreeOne))->Start();
	CurrentThread->OwnerProcess->GetThread(CreateThread(&TestThreeTwo))->Start();
	CurrentThread->OwnerProcess->GetThread(CreateThread(&TestThreeThree))->Start();
...
__attribute__((noinline)) void TestFour()
{
	for(int j = 0; j < 10; j++)
	{
		for(int i = 0; i < 10; i++)
		{
			...
			int Test = CreateThread(&TestFourProgram);

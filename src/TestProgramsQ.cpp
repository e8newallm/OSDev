...
{
	CurrentThread->OwnerProcess->GetThread(CreateQThread(&TestThreeOne, 100))->Start();
	CurrentThread->OwnerProcess->GetThread(CreateQThread(&TestThreeTwo, 150))->Start();
	CurrentThread->OwnerProcess->GetThread(CreateQThread(&TestThreeThree, 200))->Start();
...
__attribute__((noinline)) void TestFour()
{
	for(int j = 0; j < 10; j++)
	{
		for(int i = 0; i < 10; i++)
		{
			...
			int Test = CreateQThread(&TestFourProgram, 100);
